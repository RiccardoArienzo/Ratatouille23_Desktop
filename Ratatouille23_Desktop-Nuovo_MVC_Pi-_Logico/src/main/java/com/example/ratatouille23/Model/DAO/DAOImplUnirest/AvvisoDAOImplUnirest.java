...
//            ArrayList<Avviso> avvisi = new Gson().fromJson(apiResponse.getBody().toString(), avvisoListType);
    ...
    @Override
    public List<UtenteAvviso> getListUtenteAvviso(UtenteDTO utenteDTO) {
        try {
            Gson gson = new Gson();
            String utenteJson = gson.toJson(utenteDTO);
//            System.out.println("CHEST è L'UTENTE: " + utenteDTO);
            ...
            HttpResponse<JsonNode> apiResponse = Unirest.get("http://localhost:8080/api/v1/avviso/getUtenteAvviso")
//                    .queryString("utenteDTO", utenteDTO)
                    .queryString("utenteDTO", utenteJson)
                    .asJson();

            Type utenteAvvisoListType = new TypeToken<ArrayList<UtenteAvviso>>(){}.getType();
//            ArrayList<Avviso> avvisi = new Gson().fromJson(apiResponse.getBody().toString(), avvisoListType);
            ArrayList<UtenteAvviso> utenteAvvisi = gson.fromJson(apiResponse.getBody().toString(), utenteAvvisoListType);

            for(UtenteAvviso utenteAvviso : utenteAvvisi){
                System.out.println("Avviso : " + utenteAvviso);
            }
