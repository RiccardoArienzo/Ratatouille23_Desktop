package com.example.ratatouille23.View.AddettoSala;

import com.example.ratatouille23.Controller.AddettoSala.HomepageSalaController;
import com.example.ratatouille23.Homepage;
import com.example.ratatouille23.Model.Utente;
import com.example.ratatouille23.ViewInterface;
import javafx.fxml.FXML;
import javafx.fxml.FXMLLoader;
import javafx.scene.Node;
import javafx.scene.Parent;
import javafx.scene.Scene;
import javafx.scene.control.Button;
import javafx.scene.control.Label;
import javafx.scene.layout.BorderPane;
import javafx.scene.layout.VBox;
import javafx.stage.Stage;

import java.io.IOException;

public class HomepageSalaView extends Homepage implements ViewInterface {

    @FXML
    private BorderPane borderPane;
    @FXML
    private Label labelUsername;
    @FXML
    private Label labelAvvisiDisponibili;
    @FXML
    private Button btnGestisciAvvisi;
    @FXML
    private Button btnGestisciOrdinazioni;

    private Stage stage;
    private Scene scene;
    private Parent root;

    private Node node;

    private HomepageSalaController homepageSalaController;

    // *********************

    public HomepageSalaView(){};

    @FXML
    public void initialize(){
        labelUsername.setText(Utente.getUsername());
        this.homepageSalaController = new HomepageSalaController(this);
    }


    // Event Handler

    public void clickBtnGestisciAvvisi(){
        homepageSalaController.onGestisciAvvisiClicked();
    }

    public void clickBtnRegistraOrdinazioni(){
        homepageSalaController.onRegistraOrdinazioniClicked();
    }

    // Metodi di View Interface

    @Override
    public Node getNode(){
        return this.root;
    }

    @Override
    public void setNode(Node node){
        this.node = node;
    }

}
